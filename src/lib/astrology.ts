...
export interface HouseCusp {
  ...
  signIndex: number; sign: string; degree: number;
}
...
export function calcWesternChart(
  ...
  const houses: HouseCusp[] = cuspLons.map((lon, i) => {
    const z = zodiacInfo(lon);
    return { house: i + 1, longitude: Math.round(lon * 100) / 100, signIndex: z.signIndex, sign: z.sign, degree: z.degree };
  });
...
const SAJU_TODAY_FORTUNE: Record<string, { keyword: string; summary: string }> = {
  비견: { keyword: '자립',   summary: '자신감이 넘치고 독립적인 행동이 빛나는 날입니다. 새로운 시작을 두려워하지 마세요.' },
  겁재: { keyword: '경쟁',   summary: '경쟁과 갈등 상황에 주의가 필요합니다. 충동적 결정보다 신중함이 필요한 날입니다.' },
  식신: { keyword: '즐거움', summary: '창의력과 여유가 넘치는 하루입니다. 좋아하는 것을 즐기고 맛있는 것을 드세요.' },
  상관: { keyword: '표현',   summary: '자기표현 욕구가 강한 날입니다. 말 한마디가 관계를 좌우할 수 있으니 신중히 하세요.' },
  편재: { keyword: '기회',   summary: '뜻밖의 기회나 수입이 찾아올 수 있습니다. 능동적으로 움직이면 좋은 결과가 따릅니다.' },
  정재: { keyword: '안정',   summary: '꾸준한 노력이 결실을 맺는 날입니다. 재물과 관련된 안정적인 성과를 기대해 보세요.' },
  편관: { keyword: '긴장',   summary: '예상치 못한 압박이 찾아올 수 있습니다. 건강 관리와 스트레스 해소에 신경 쓰세요.' },
  정관: { keyword: '질서',   summary: '원칙과 신뢰가 중요한 날입니다. 공식적인 자리나 업무에서 좋은 인상을 줄 수 있어요.' },
  편인: { keyword: '직관',   summary: '직관과 영감이 풍부한 날입니다. 혼자만의 시간을 가지며 아이디어를 정리해 보세요.' },
  정인: { keyword: '배움',   summary: '학습과 지혜가 빛나는 날입니다. 새로운 지식을 받아들이거나 조언을 구하면 도움이 됩니다.' },
};
...
export function calcTodaySajuFortune(saju: SajuResult, lmtOffsetMin: number = 0): {
  ...
  const fortune = SAJU_TODAY_FORTUNE[sipShin] ?? { keyword: '평온', summary: '평범하지만 안정적인 하루입니다.' };
  return {
    ...fortune,
    sipShin,
...
const WESTERN_TODAY_FORTUNE: Record<number, { keyword: string; summary: string }> = {
  0:  { keyword: '공명',   summary: '달이 태어날 때와 같은 자리입니다. 내면이 편안하고 감정이 선명하게 느껴지는 날입니다.' },
  1:  { keyword: '설렘',   summary: '새로운 감정의 흐름이 시작됩니다. 주변 변화에 유연하게 반응하면 좋은 날입니다.' },
  2:  { keyword: '소통',   summary: '활발한 소통과 정보 교류가 이루어지는 날입니다. 대화를 통해 좋은 아이디어가 나올 수 있어요.' },
  3:  { keyword: '성장',   summary: '감정적 에너지가 충만한 날입니다. 중요한 결정보다 내면의 소리에 귀 기울여 보세요.' },
  4:  { keyword: '조화',   summary: '감정과 현실이 조화를 이루는 날입니다. 창의적 표현이 빛을 발합니다.' },
  5:  { keyword: '성찰',   summary: '조용히 돌아보는 시간이 필요한 날입니다. 완성에 가까워지는 느낌을 받을 수 있어요.' },
  6:  { keyword: '전환',   summary: '감정적 전환점이 찾아오는 날입니다. 오래된 패턴을 내려놓을 좋은 기회입니다.' },
  7:  { keyword: '해소',   summary: '내면의 긴장이 서서히 풀리는 날입니다. 내가 원하는 것이 무엇인지 선명해집니다.' },
  8:  { keyword: '재충전', summary: '에너지를 재충전하고 새로운 방향을 모색하기 좋은 날입니다.' },
  9:  { keyword: '직관',   summary: '직관과 통찰이 날카로워지는 날입니다. 느낌을 믿고 행동해 보세요.' },
  10: { keyword: '유연',   summary: '유연한 사고와 감정 조절이 돋보이는 날입니다. 다양한 관점을 수용해 보세요.' },
  11: { keyword: '마무리', summary: '한 사이클이 마무리되는 시점입니다. 감사와 정리로 다음을 준비하세요.' },
};
...
export function calcTodayWesternFortune(western: WesternChartResult): {
  ...
  const fortune = WESTERN_TODAY_FORTUNE[diff] ?? { keyword: '평온', summary: '감정적으로 안정적인 하루를 보낼 수 있습니다.' };
  return {
    ...fortune,
